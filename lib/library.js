(function() {
  ...
  exports.HappyHands = (function() {
    ...
    function HappyHands(options) {
      var _this = this;
      this.checks = 0;
      if (options) {
        this.accuracy = options.accuracy || 20;
  ...
  Listener = (function() {
    ...
    function Listener(records, parent, callback, options) {
      ...
      if (options) {
        this.accuracy = options.accuracy || this.parent.accuracy;
        this.kill_time = options.kill_time || 1000;
        this.kill_on_complete = options.kill_on_complete || false;
      }
      this.complete = false;
      this.current_pos = 0;
      this.passes = 0;
      ...
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        ...
        if (Math.abs(point - this.parent.position[_i]) < this.accuracy) {
          ...
          if (this.passes >= 5) {
            ...
            this.kill_timeout = setTimeout(function() {
              return _this.current_pos = 0;
            }, this.kill_time);
            this.current_pos++;
          }
        }
      }
      if (Math.abs(this.current_pos - this.records.length) < 5) {
        this.time = new Date().getTime() - start_time;
        this.current_pos = 0;
        this.callback();
        if (this.kill_on_complete) return this.remove_from_parent();
      }
    };
